...
export const ContactDetailDrawer: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-end bg-black/70 backdrop-blur-xs">
      <div className="bg-[#121215] w-full max-w-2xl h-full shadow-2xl flex flex-col border-l border-zinc-800 animate-in slide-in-from-right duration-200">
        {/* Drawer Header */}
        <div className="p-6 border-b border-zinc-800 bg-[#161619] shrink-0">
            ...
            <div className="flex items-start gap-4">
              {contact.profileImage ? (
                <img
                  ...
                  className="w-14 h-14 rounded-2xl object-cover border-2 border-zinc-700 shadow-md"
                />
              ) : (
              ...
              <div>
                ...
                <div className="text-xs font-semibold text-zinc-400 mt-0.5">{contact.jobTitle}</div>
                <button
                  onClick={() => {
          ...
          <div className="flex items-center gap-2 mt-4 pt-4 border-t border-zinc-800">
            <button
              ...
              className="flex-1 py-2 px-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5 shadow-xs transition-all cursor-pointer"
            >
            ...
            {contact.phone && (
              <a
                ...
                className="py-2 px-3 bg-[#121215] hover:bg-zinc-800 text-zinc-300 rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5 transition-all border border-zinc-800"
              >
        ...
        {/* Tab Navigation */}
        <div className="px-6 border-b border-zinc-800 flex items-center gap-2 shrink-0 bg-[#161619]">
          {[
        ...
        {/* Content */}
        <div className="p-6 overflow-y-auto flex-1 text-xs space-y-5 text-zinc-300">
          {activeTab === 'profile' && (
            <div className="space-y-4">
              <div className="p-4 bg-[#161619] border border-zinc-800 rounded-xl space-y-3">
                <div className="font-bold text-zinc-100 uppercase tracking-wider text-xs">
                  Contact Information
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                  <div>
                    <div className="text-zinc-500 text-[11px]">Email Address</div>
                    <a
                  ...
                  <div>
                    <div className="text-zinc-500 text-[11px]">Office Phone</div>
                    <div className="font-semibold text-zinc-200">{contact.phone || 'None'}</div>
                  </div>
                  <div>
                    <div className="text-zinc-500 text-[11px]">Mobile Phone</div>
                    <div className="font-semibold text-zinc-200">{contact.mobile || 'None'}</div>
                  </div>
                  <div>
                    <div className="text-zinc-500 text-[11px]">Preferred Channel</div>
                    <div className="font-semibold text-zinc-200">
                      {contact.preferredContactMethod}
                    </div>
                  </div>
                  <div>
                    <div className="text-zinc-500 text-[11px]">Department</div>
                    <div className="font-semibold text-zinc-200">
                      {contact.department || 'General'}
                    </div>
                  </div>
                  <div>
                    <div className="text-zinc-500 text-[11px]">Lead Source</div>
                    <div className="font-semibold text-zinc-200">{contact.source || 'Direct'}</div>
                  </div>
                </div>
              </div>
          ...
          {activeTab === 'opportunities' && (
            <div className="space-y-3">
              <div className="font-bold text-zinc-400 uppercase text-xs">Associated Deals</div>
          ...
          {activeTab === 'activities' && (
            <div className="space-y-3">
              <div className="font-bold text-zinc-400 uppercase text-xs">
