...
export const OpportunityDetailModal: React.FC = () => {
  ...
  // Retrieve freshest opportunity state
  ...
  const relatedNotes = notes.filter(
    (n) => n.relatedToType === 'Opportunity' && n.relatedToId === opp.id
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-xs">
      <div className="bg-[#161619] rounded-2xl border border-zinc-800 shadow-2xl max-w-4xl w-full overflow-hidden animate-in fade-in zoom-in-95 duration-150 max-h-[94vh] flex flex-col">
        {/* Header */}
        <div className="p-6 border-b border-zinc-800 flex items-start justify-between bg-[#121215] shrink-0">
          <div className="flex items-start gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h2 className="text-xl font-bold text-zinc-100 tracking-tight">{opp.name}</h2>
                <span
              ...
              <div className="text-xs text-zinc-400 mt-1 flex items-center gap-4 flex-wrap">
                <button
                  onClick={() => {
                    setSelectedAccountIdFor360(opp.accountId);
                    setSelectedOpportunityForDetail(null);
                  }}
                  className="flex items-center gap-1 text-blue-400 hover:underline font-medium cursor-pointer"
                >
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="p-2 text-zinc-300 hover:text-zinc-100 hover:bg-zinc-800 rounded-lg transition-colors cursor-pointer text-xs font-medium flex items-center gap-1 border border-zinc-800"
            >
        ...
        {/* Salesforce-Style Stage Chevron Path */}
        <div className="px-6 py-3 bg-[#121215] border-b border-zinc-800 overflow-x-auto shrink-0">
                ...
                <button
                  ...
                  className={`flex-1 py-2 px-3 text-xs font-semibold rounded-md transition-all flex items-center justify-center gap-1.5 cursor-pointer ${
                    isCurrent
                      ...
                      : 'bg-[#161619] text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800 border border-zinc-800'
                  }`}
                >
            ...
            <button
              ...
              className={`py-2 px-3 text-xs font-semibold rounded-md transition-all flex items-center justify-center gap-1 cursor-pointer ${
                opp.stage === 'Closed Lost'
                  ...
                  : 'bg-[#161619] text-rose-400 hover:bg-rose-950/30 border border-rose-900/40'
              }`}
            >
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6 text-xs">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="p-3.5 bg-[#121215] border border-zinc-800 rounded-xl">
              <div className="text-[11px] font-semibold text-zinc-500 uppercase tracking-wider">Deal Value</div>
              <div className="text-xl font-bold text-zinc-100 mt-0.5">${opp.amount.toLocaleString()}</div>
            </div>
            <div className="p-3.5 bg-[#121215] border border-zinc-800 rounded-xl">
              <div className="text-[11px] font-semibold text-zinc-500 uppercase tracking-wider">Probability</div>
              <div className="text-xl font-bold text-blue-400 mt-0.5">{opp.probability}%</div>
            </div>
            <div className="p-3.5 bg-[#121215] border border-zinc-800 rounded-xl">
              <div className="text-[11px] font-semibold text-zinc-500 uppercase tracking-wider">Expected Revenue</div>
              <div className="text-xl font-bold text-emerald-400 mt-0.5">
                ${opp.expectedRevenue.toLocaleString()}
              </div>
            </div>
            <div className="p-3.5 bg-[#121215] border border-zinc-800 rounded-xl">
              <div className="text-[11px] font-semibold text-zinc-500 uppercase tracking-wider">Lead Source</div>
              <div className="text-sm font-semibold text-zinc-200 mt-1 truncate">{opp.leadSource}</div>
            </div>
          </div>
          ...
          {opp.wonLostReason && (
            ...
            >
              <div className="flex items-center gap-2 font-bold text-xs uppercase tracking-wider mb-1">
                {opp.status === 'Won' ? <Trophy className="w-4 h-4 text-emerald-400" /> : <AlertTriangle className="w-4 h-4 text-rose-400" />}
                {opp.status === 'Won' ? 'Deal Win Driver' : 'Closed Lost Reason'}
              </div>
              <div className="text-sm font-medium">{opp.wonLostReason}</div>
              {opp.actualCloseDate && (
                <div className="text-xs text-zinc-400 mt-1">Closed Date: {opp.actualCloseDate}</div>
              )}
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="p-4 bg-[#121215] border border-zinc-800 rounded-xl">
              ...
              {account ? (
                <div>
                  <h4 className="text-sm font-bold text-zinc-100">{account.name}</h4>
                  <p className="text-xs text-zinc-400 mt-0.5">
                    {account.industry} • {account.businessSegment} • {(account.annualRevenue / 1000000).toFixed(1)}M Rev
                  </p>
                  <p className="text-xs text-zinc-400 mt-2 line-clamp-2">{account.description}</p>
                </div>
              ) : (
                <div className="text-xs text-zinc-500">Linked to account ID: {opp.accountId}</div>
              )}
            </div>
                ...
                <div>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-zinc-400">{contact.jobTitle}</p>
                    </div>
                  </div>
                  <div className="mt-3 flex items-center gap-3 text-xs">
                    ...
                    {contact.phone && (
                      <a href={`tel:${contact.phone}`} className="text-zinc-400 flex items-center gap-1">
          ...
          <div className="p-4 bg-blue-950/20 border border-blue-900/40 rounded-xl">
            <div className="text-xs font-bold text-blue-400 uppercase tracking-wider mb-1 flex items-center gap-1.5">
              <ArrowRight className="w-3.5 h-3.5 text-blue-400" /> Active Next Step
            </div>
          ...
          <div className="p-5 bg-indigo-950/20 border border-indigo-900/40 rounded-2xl">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h4 className="text-sm font-bold text-zinc-100">AI Deal Coach & Opportunity Advisor</h4>
                  <p className="text-xs text-indigo-400">Powered by Gemini 3.7 Flash Deal Intelligence</p>
                </div>
              </div>
              <button
                ...
                disabled={loadingAi}
                className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-xs font-semibold shadow-xs transition-all active:scale-[0.98] cursor-pointer disabled:opacity-50 flex items-center gap-1.5"
              >
            ...
            {aiInsight ? (
              <div className="space-y-4 pt-2">
                <div className="p-3 bg-[#121215] border border-zinc-800 rounded-xl text-xs text-zinc-200 leading-relaxed font-medium">
                  {aiInsight.summary}
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div className="p-3 bg-emerald-950/30 border border-emerald-800/50 rounded-xl">
                    <div className="text-xs font-bold text-emerald-300 uppercase tracking-wider mb-2 flex items-center gap-1">
                      <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" /> Win Accelerators
                    </div>
                  ...
                  <div className="p-3 bg-amber-950/30 border border-amber-800/50 rounded-xl">
                    <div className="text-xs font-bold text-amber-300 uppercase tracking-wider mb-2 flex items-center gap-1">
                      <AlertTriangle className="w-3.5 h-3.5 text-amber-400" /> Deal Risk Flags
                    </div>
                ...
                {aiInsight.suggestedNextSteps && (
                  <div className="p-3 bg-[#121215] border border-zinc-800 rounded-xl">
                    <div className="text-xs font-bold text-zinc-100 uppercase tracking-wider mb-1.5">
                      Recommended Strategic Actions
                    </div>
                    <div className="space-y-1 text-xs text-zinc-300">
                      {aiInsight.suggestedNextSteps.map((step, i) => (
                        <div key={i} className="flex items-center gap-2">
                          <span className="w-4 h-4 rounded-full bg-indigo-500/20 text-indigo-400 text-[10px] font-bold flex items-center justify-center shrink-0">
                            {i + 1}
                          </span>
                          <span>{step}</span>
                        </div>
          ...
          <div>
            <div className="text-xs font-bold text-zinc-400 uppercase tracking-wider mb-3 flex items-center gap-1.5">
              <Clock className="w-3.5 h-3.5 text-zinc-500" /> Opportunity History & Milestones
            </div>
            {relatedActivities.length === 0 ? (
              ...
              <div className="space-y-2">
                {relatedActivities.map((act) => (
                  <div
                    key={act.id}
                    className="p-3 bg-[#121215] border border-zinc-800 rounded-xl flex items-start gap-3 text-xs"
                  >
