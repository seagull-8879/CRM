...
import { OpportunityStage, Opportunity, OpportunityStatus } from '../../types';
...
export const HomeView: React.FC = () => {
  ...
  return (
    <div id="home-dashboard-view" className="p-6 space-y-6 max-w-7xl mx-auto">
      {/* Welcome Banner */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-[#161619] via-[#1A1A22] to-[#161619] text-white p-6 rounded-2xl shadow-xl border border-zinc-800">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs uppercase tracking-widest font-bold text-blue-400 bg-blue-500/10 px-2 py-0.5 rounded border border-blue-500/20">
              Welcome back
            </span>
            <span className="text-xs text-zinc-400">• {new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric', year: 'numeric' })}</span>
          </div>
          <h1 className="text-2xl font-bold tracking-tight text-zinc-100">
            Hello, {currentUser?.name || 'Sales Professional'}
          </h1>
          <p className="text-xs text-zinc-400 mt-1 max-w-xl">
            Here is your live opportunity pipeline and performance overview. You have {openDeals.length} active deals representing ${openPipelineValue.toLocaleString()} in total pipeline value.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-4 py-2.5 bg-indigo-600/20 hover:bg-indigo-600/30 text-indigo-300 rounded-xl text-xs font-semibold flex items-center gap-2 border border-indigo-500/30 shadow-sm transition-all active:scale-[0.98] cursor-pointer"
          >
          ...
          <button
            ...
            className="px-4 py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-semibold flex items-center gap-2 shadow-lg shadow-blue-600/30 transition-all active:scale-[0.98] cursor-pointer"
          >
      ...
      {/* KPI Cards (BRD FR-004: Open, Won, Lost, Pipeline Value) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-5 bg-[#161619] border border-zinc-800 rounded-2xl shadow-xs">
          ...
          <div className="flex items-center gap-2 mt-2 text-xs text-zinc-400">
            <span className="font-semibold text-blue-400">{openDeals.length} Open Deals</span>
        ...
        <div className="p-5 bg-[#161619] border border-zinc-800 rounded-2xl shadow-xs">
          ...
          <div className="flex items-center gap-2 mt-2 text-xs text-zinc-400">
            <span className="font-semibold text-emerald-400">{wonDeals.length} Deals Won</span>
        ...
        <div className="p-5 bg-[#161619] border border-zinc-800 rounded-2xl shadow-xs">
          ...
          <div className="flex items-center gap-2 mt-2 text-xs text-zinc-400">
            <span className="font-semibold text-rose-400">{lostDeals.length} Deals Lost</span>
        ...
        <div className="p-5 bg-[#161619] border border-zinc-800 rounded-2xl shadow-xs">
          ...
          <div className="flex items-center gap-2 mt-2 text-xs text-zinc-400">
            <span className="font-semibold text-indigo-400">{accounts.length} Accounts</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Primary Opportunity Workspace (FR-005: On Home, no separate sidebar item) */}
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-[#161619] border border-zinc-800 rounded-2xl shadow-xs p-5">
            {/* Header & Controls */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-zinc-800">
              <div>
                <h2 className="text-base font-bold text-zinc-100">Opportunities Workspace</h2>
              ...
              <div className="flex items-center gap-2">
                <div className="flex items-center bg-[#121215] p-1 rounded-lg border border-zinc-800">
                  <button
                    onClick={() => setViewMode('table')}
                    className={`p-1.5 rounded-md text-xs font-medium transition-all cursor-pointer ${
                      viewMode === 'table' ? 'bg-zinc-800 text-zinc-100 shadow-xs' : 'text-zinc-400 hover:text-zinc-200'
                    }`}
                    title="Table View"
                  >
                  ...
                  <button
                    ...
                    className={`p-1.5 rounded-md text-xs font-medium transition-all cursor-pointer ${
                      viewMode === 'kanban' ? 'bg-zinc-800 text-zinc-100 shadow-xs' : 'text-zinc-400 hover:text-zinc-200'
                    }`}
                ...
                <button
                  ...
                  className="px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-xs font-semibold shadow-xs flex items-center gap-1 transition-all cursor-pointer"
                >
            ...
            <div className="pt-3 pb-1 flex flex-wrap items-center gap-2.5">
              <div className="relative flex-1 min-w-[180px]">
                ...
                <input
                  ...
                  className="w-full pl-8.5 pr-3 py-1.5 bg-[#121215] border border-zinc-800 rounded-lg text-xs text-zinc-100 placeholder:text-zinc-500 focus:outline-hidden focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                />
              </div>
              ...
              <select
                ...
                className="px-2.5 py-1.5 bg-[#121215] border border-zinc-800 rounded-lg text-xs text-zinc-300 focus:outline-hidden focus:ring-2 focus:ring-blue-500/20"
              >
                  ...
                  <tbody className="divide-y divide-zinc-800/80">
                    {filteredOpportunities.length === 0 ? (
                      ...
                      filteredOpportunities.map((opp) => (
                        ...
                        >
                          <td className="py-3 px-4 font-semibold text-zinc-100 max-w-[200px]">
                            <div className="truncate group-hover:text-blue-400 transition-colors">
                              {opp.name}
                            </div>
                          ...
                          <td className="py-3 px-4 max-w-[180px]">
                            <button
                              ...
                              className="text-blue-400 hover:underline font-medium truncate block text-left"
                            >
                              {opp.accountName}
                            </button>
                              ...
                              {opp.status === 'Open' && (
                                <>
                                  <button
                                    ...
                                    title="Mark Won"
                                    className="p-1 text-emerald-400 hover:bg-emerald-500/15 rounded-md transition-colors cursor-pointer"
                                  >
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-3 xl:grid-cols-6 gap-3 overflow-x-auto pb-2">
                {stagesList.map((stg) => {
                  ...
                  return (
                    <div
                      key={stg}
                      className="bg-[#121215] border border-zinc-800 rounded-xl p-3 flex flex-col min-w-[200px]"
                    >
                      <div className="flex items-center justify-between pb-2 border-b border-zinc-800 mb-2.5">
                        <div className="font-bold text-xs text-zinc-200 truncate">{stg}</div>
                        <span className="text-[10px] font-semibold bg-[#18181B] border border-zinc-800 px-1.5 py-0.5 rounded text-zinc-300">
                          {stageOpps.length}
                        </span>
                      </div>
                      ...
                      <div className="space-y-2 flex-1 overflow-y-auto max-h-[460px]">
                        {stageOpps.map((opp) => (
                          <div
                            key={opp.id}
                            onClick={() => setSelectedOpportunityForDetail(opp)}
                            className="p-2.5 bg-[#18181B] border border-zinc-800 rounded-lg shadow-2xs hover:border-blue-500/50 hover:bg-[#1F1F24] transition-all cursor-pointer group"
                          >
                            <div className="font-semibold text-xs text-zinc-100 group-hover:text-blue-400 line-clamp-2">
                              {opp.name}
                            </div>
                            ...
                            <div className="mt-2 pt-2 border-t border-zinc-800/80 flex items-center justify-between text-[11px]">
                              ...
                              <span className="text-zinc-500">{opp.expectedCloseDate}</span>
                            </div>
                          </div>
                        ))}
                      </div>
        ...
        {/* Right Column: Upcoming Tasks & Recent Activities (FR-007) */}
        <div className="space-y-6">
          {/* Upcoming Tasks Widget */}
          <div className="bg-[#161619] border border-zinc-800 rounded-2xl shadow-xs p-5">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded-lg bg-blue-500/10 text-blue-400 border border-blue-500/20 flex items-center justify-center font-bold">
                  <CheckSquare className="w-4 h-4" />
                </div>
                <div>
                  <h3 className="text-sm font-bold text-zinc-100">Upcoming Tasks</h3>
                  <p className="text-[11px] text-zinc-400">Action items and follow-ups</p>
                </div>
              </div>
            ...
            <div className="space-y-2.5">
              {pendingTasks.length === 0 ? (
                ...
                pendingTasks.map((t) => (
                  <div
                    key={t.id}
                    className="p-3 bg-[#121215] hover:bg-zinc-800/40 border border-zinc-800 rounded-xl flex items-start gap-2.5 transition-colors group"
                  >
                    <input
                      type="checkbox"
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-xs font-semibold text-zinc-100 line-clamp-2">
                        {t.title}
                      </div>
          ...
          <div className="bg-[#161619] border border-zinc-800 rounded-2xl shadow-xs p-5">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded-lg bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 flex items-center justify-center font-bold">
                  <Clock className="w-4 h-4" />
                </div>
                <div>
                  <h3 className="text-sm font-bold text-zinc-100">Recent Activities</h3>
                  <p className="text-[11px] text-zinc-400">Real-time CRM audit stream</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3 relative before:absolute before:left-2 before:top-2 before:bottom-2 before:w-0.5 before:bg-zinc-800">
              {recentActivityList.map((act) => (
                <div key={act.id} className="relative pl-6 text-xs">
                  <div className="absolute left-1 top-1.5 w-2.5 h-2.5 rounded-full bg-blue-500 border-2 border-[#161619]" />
                  <div className="font-semibold text-zinc-100">{act.title}</div>
                  <div className="text-zinc-400 mt-0.5 text-[11px] line-clamp-2">
                    {act.description}
                  </div>
