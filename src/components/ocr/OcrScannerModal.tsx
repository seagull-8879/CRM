...
export const OcrScannerModal: React.FC = () => {
  ...
  const processImageWithGemini = async (imageData: string) => {
    ...
    try {
      ...
      console.error(err);
      // Fallback extracted fields if API key isn't active
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-xs">
      <div className="bg-[#161619] rounded-2xl border border-zinc-800 shadow-2xl max-w-3xl w-full overflow-hidden animate-in fade-in zoom-in-95 duration-150 max-h-[92vh] flex flex-col">
          ...
          <div className="p-6 overflow-y-auto flex-1 space-y-6 text-xs">
              ...
              <button
                ...
                className={`py-2 px-5 rounded-xl font-semibold flex items-center gap-2 transition-all cursor-pointer ${
                  mode === 'upload'
                    ...
                    : 'bg-[#121215] text-zinc-400 hover:text-zinc-200 border border-zinc-800 hover:bg-[#1C1C20]'
                }`}
              >
            ...
            {mode === 'upload' && (
              <div className="space-y-4">
                <label className="border-2 border-dashed border-zinc-800 hover:border-blue-500 rounded-2xl p-8 flex flex-col items-center justify-center text-center cursor-pointer transition-colors bg-[#121215] hover:bg-[#1A1A20] group">
                  <div className="w-14 h-14 rounded-2xl bg-blue-500/10 group-hover:bg-blue-500/20 text-blue-400 flex items-center justify-center mb-3 transition-colors">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {demoCards.map((card, idx) => (
                      <button
                        ...
                        className="p-3 bg-[#121215] hover:bg-[#1C1C20] border border-zinc-800 hover:border-zinc-700 rounded-xl text-left transition-all flex items-center gap-3 cursor-pointer group"
                      >
                        <img
                          src={card.url}
                          alt={card.label}
                          className="w-12 h-8 object-cover rounded-md border border-zinc-700 shrink-0"
                        />
                        <div>
                          ...
                          <div className="text-[11px] text-zinc-400">{card.sampleData.companyName}</div>
                        </div>
                      </button>
                    ))}
                  </div>
            ...
            {mode === 'camera' && (
              <div className="space-y-4 flex flex-col items-center">
                {cameraError ? (
                  <div className="p-4 bg-amber-950/30 border border-amber-800/60 rounded-xl text-amber-300 text-xs">
                    {cameraError}
                  </div>
                ) : (
                  <div className="relative rounded-2xl overflow-hidden bg-black border border-zinc-700 w-full max-w-md aspect-video flex items-center justify-center">
                ...
                {cameraActive && (
                  <button
                    ...
                    className="py-2.5 px-6 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold flex items-center gap-2 shadow-lg shadow-blue-600/30 active:scale-95 transition-all cursor-pointer"
                  >
                    <Camera className="w-4 h-4" />
            ...
            {isProcessing && (
              <div className="p-6 bg-indigo-950/30 border border-indigo-800/60 rounded-2xl flex items-center justify-center gap-3 text-indigo-300">
        ...
        {/* Step 2: Review & Commit Extracted Data (FR-018, FR-019) */}
        {step === 'review' && (
          <div className="p-6 overflow-y-auto flex-1 space-y-6 text-xs">
            <div className="p-3 bg-emerald-950/30 border border-emerald-800/60 rounded-xl text-emerald-300 flex items-center gap-2 font-medium">
            ...
            <div>
              <div className="font-bold text-zinc-500 uppercase tracking-wider text-[11px] mb-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                ...
                <div>
                  <label className="block font-semibold text-zinc-300 mb-1">Mobile</label>
                  <input
                    type="text"
                    value={mobile}
            ...
            <div className="pt-4 border-t border-zinc-800">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                <div>
                  <label className="block font-semibold text-zinc-300 mb-1">Company / Organization</label>
                  <input
                    type="text"
                    value={companyName}
              ...
              <div className="mt-3 p-3.5 bg-[#121215] border border-zinc-800 rounded-xl space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <label htmlFor="opt_new_account" className="font-semibold text-zinc-200 cursor-pointer">
                    Create new Client Account: <span className="text-blue-400">"{companyName || 'New Account'}"</span>
                  </label>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="accent-blue-600"
                  />
                  <label htmlFor="opt_existing_account" className="font-semibold text-zinc-200 cursor-pointer">
                ...
                {!createAccountIfNew && (
                  <select
                    ...
                    className="w-full mt-2 px-3 py-1.5 bg-[#161619] border border-zinc-800 rounded-lg text-xs text-zinc-100 focus:outline-hidden focus:ring-2 focus:ring-blue-500/20"
                  >
            ...
            <div className="pt-4 border-t border-zinc-800">
              <div className="flex items-center justify-between mb-3">
                ...
                <label className="flex items-center gap-1.5 font-semibold text-zinc-300 cursor-pointer">
                  <input
                    type="checkbox"
              ...
              {createOpportunity && (
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 p-3.5 bg-blue-950/20 border border-blue-900/40 rounded-xl">
                  ...
                  <div>
                    <label className="block text-[11px] font-semibold text-zinc-300 mb-1">Stage</label>
                    <select
        ...
        {step === 'success' && (
          <div className="p-8 text-center space-y-4 my-auto">
            <div className="w-16 h-16 rounded-2xl bg-emerald-950/40 border border-emerald-800/80 text-emerald-400 flex items-center justify-center mx-auto shadow-sm">
              <Check className="w-8 h-8" />
            </div>
            <div>
              <h3 className="text-lg font-bold text-zinc-100">Card Processed & Saved!</h3>
              <p className="text-xs text-zinc-400 mt-1 max-w-md mx-auto">
                Contact <span className="font-semibold text-zinc-200">{firstName} {lastName}</span> and company <span className="font-semibold text-zinc-200">{companyName}</span> have been synchronized into your CRM.
              </p>
            </div>
            <button
              onClick={() => setIsOcrScannerOpen(false)}
              className="px-6 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-semibold shadow-md cursor-pointer"
            >
          ...
          <div className="p-4 border-t border-zinc-800 flex items-center justify-between bg-[#121215] shrink-0">
            {step === 'review' ? (
              <button
                ...
                className="px-4 py-2 font-semibold text-zinc-300 hover:bg-zinc-800 rounded-lg text-xs cursor-pointer"
              >
                Back to Scanner
              </button>
            ) : (
            ...
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={() => setIsOcrScannerOpen(false)}
                className="px-4 py-2 font-semibold text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800 rounded-lg text-xs cursor-pointer"
              >
                Cancel
              </button>
              {step === 'review' && (
                <button
                  ...
                  className="px-5 py-2 font-semibold text-white bg-blue-600 hover:bg-blue-500 rounded-lg text-xs shadow-md shadow-blue-600/20 active:scale-98 transition-all cursor-pointer"
                >
