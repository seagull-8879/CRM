...
export const Header: React.FC = () => {
  ...
  return (
    <header
      id="crm-header"
      className="h-16 bg-white/95 dark:bg-[#0E121E]/95 backdrop-blur-md border-b border-slate-200 dark:border-slate-800/80 px-6 flex items-center justify-between sticky top-0 z-30 shadow-xs text-slate-800 dark:text-slate-100 transition-colors duration-200"
    >
      ...
      <div className="flex-1 max-w-xl relative" ref={searchRef}>
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-slate-100/80 hover:bg-slate-100 focus:bg-white dark:bg-[#151B2B] dark:hover:bg-[#1A2236] dark:focus:bg-[#151B2B] border border-slate-200 dark:border-slate-800/80 rounded-xl text-sm text-slate-800 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-hidden focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all"
          />
          {searchQuery && (
            <button
              onClick={() => setSearchQuery('')}
              className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 px-1 font-medium"
            >
              Clear
            </button>
          )}
                  ...
                  <div className="p-2">
                    ...
                    {filteredAccounts.map((acc) => (
                      <button
                        ...
                        className="w-full text-left px-3 py-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800/60 flex items-center justify-between text-sm transition-colors cursor-pointer"
                      >
                        <div>
                          <div className="font-semibold text-slate-900 dark:text-slate-100">{acc.name}</div>
                          <div className="text-xs text-slate-500 dark:text-slate-400">
                            {acc.industry} • {acc.businessSegment}
                          </div>
                        </div>
                        <span className="text-xs font-semibold text-blue-600 dark:text-blue-400 bg-blue-500/10 border border-blue-500/20 px-2 py-0.5 rounded-lg">
                  ...
                  <div className="p-2">
                    ...
                    {filteredContacts.map((cnt) => (
                      ...
                      >
                        ...
                        <span className="text-xs text-slate-500 dark:text-slate-400">{cnt.email}</span>
                      </button>
                    ))}
                  </div>
                )}
                ...
                {filteredOpportunities.length > 0 && (
                  <div className="p-2">
                    ...
                    {filteredOpportunities.map((opp) => (
                      ...
                      >
                        <div>
                          <div className="font-semibold text-slate-900 dark:text-slate-100">{opp.name}</div>
                          <div className="text-xs text-slate-500 dark:text-slate-400">
      ...
      <div className="flex items-center gap-2.5">
        ...
        {/* Role Switcher Pill */}
        <div className="hidden sm:flex items-center bg-slate-100/90 dark:bg-[#151B2B] p-1 rounded-xl border border-slate-200 dark:border-slate-800/80 text-xs">
          <span className="text-slate-400 dark:text-slate-500 font-medium px-2 hidden md:inline">Role:</span>
        ...
        <button
          ...
          className="hidden md:flex items-center gap-1.5 px-3 py-1.5 bg-purple-500/10 hover:bg-purple-500/20 text-purple-700 dark:text-purple-300 border border-purple-500/25 rounded-xl text-xs font-semibold transition-colors cursor-pointer"
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            className="flex items-center gap-1 px-3 py-1.5 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 text-white rounded-xl text-xs font-semibold shadow-md shadow-purple-900/20 transition-all active:scale-[0.98] cursor-pointer"
          >
